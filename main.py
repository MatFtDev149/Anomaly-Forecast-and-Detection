...
def generate_synthetic_data(n_patients=100, n_days=365):
    data = []
    for patient_id in range(1, n_patients + 1):
        for day in range(n_days):
            date = pd.to_datetime('2023-01-01') + pd.to_timedelta(day, unit='D')
            vitals = np.random.normal(loc=100, scale=10)
            test_result = np.random.normal(loc=50, scale=15)
            indicator = np.random.choice([0, 1])
            data.append([patient_id, date, vitals, test_result, indicator])
    return pd.DataFrame(data, columns=['patient_id', 'date', 'vitals', 'test_result', 'indicator'])
...
scaler = scaler.fit(df[['vitals', 'test_result', 'indicator']])
df[['vitals', 'test_result', 'indicator']] = scaler.transform(df[['vitals', 'test_result', 'indicator']])
...
train_patients = df['patient_id'].unique()[:80]
test_patients = df['patient_id'].unique()[80:]
X_train, X_test, y_train, y_test = [], [], [], []

for patient_id in df['patient_id'].unique():
    patient_df = df[df['patient_id'] == patient_id]
    if patient_id in train_patients:
        for sequence in gen_sequence(patient_df, 8, ['vitals']):
            X_train.append(sequence)
        for label in gen_labels(patient_df, 8, ['vitals']):
